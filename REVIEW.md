...
|---|---|---|
| **Licence** | the first public release | **Open, and must not be decided silently.** PLAN recommends AGPL-3.0 + CLA with commercial dual-licensing; `LICENSE` and every crate manifest already say `AGPL-3.0-only`, so the recommendation is being enacted by default. That is the wrong way for this to happen. It affects contributor model, repository visibility, adoption, SaaS use and procurement, and it is **irreversible** once outside contributions arrive unsigned |
