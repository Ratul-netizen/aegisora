...
//!
//! The risk of a copy is drift, so: what a poller shuts down is not what a server shuts
//! down — the server drains requests it accepted, the poller abandons a tick it has not
//! finished — and the only shared part is *when*, which is the platform's and not
//! something either of them gets to decide. If that changes, both change.
