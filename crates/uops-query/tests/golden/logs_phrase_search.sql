SELECT tenant_id, resource_id, site_id, observed_at, ingested_at, source_kind, source_vendor, severity, facility, body, attributes, trace_id, span_id FROM logs WHERE tenant_id = {p0:UUID} AND observed_at >= {p1:DateTime64(3)} AND observed_at < {p2:DateTime64(3)} AND (searchAll(body, [{p3:String}, {p4:String}, {p5:String}, {p6:String}]) AND positionCaseInsensitive(body, {p7:String}) > 0) ORDER BY resource_id ASC, observed_at DESC LIMIT 100
